...
###############################################################################
# mods for multiprocessing 2026, cm
...
def set_console_title(proc_id=None):
    try:
        if "-proc" in sys.argv:
            idx = sys.argv.index("-proc")
            if idx + 1 < len(sys.argv):
                proc_id = sys.argv[idx + 1]
        
        if proc_id is not None:
            title = f"gdal_retile_multi - proc {proc_id}"
            # Optional: title += f"  |  {os.path.basename(sys.argv[0])}"
...
class tile_info:
    ...
    def __init__(self, xsize, ysize, tileWidth, tileHeight, overlap, totalProcs):
        self.width = xsize
        self.height = ysize
        ...
        self.lastTileWidth  = xsize - (self.countTilesX - 1) * (tileWidth - overlap)
        self.lastTileHeight = ysize - (self.countTilesY - 1) * (tileHeight - overlap)
    ...
    def report(self):
        ...
        print('total processes: %d' % self.totalProcs)
...
def tileImage(g, minfo, ti):
    ...
    yRanges = list(split(range(1, ti.countTilesY + 1), g.totalProcs))
    
    if np.array(yRanges[g.thisProc-1]).size:
        starty = yRanges[g.thisProc-1][0]
        endy = yRanges[g.thisProc-1][-1]
...
def createPyramidTile2(g, levelMosaicInfo, offsetX, offsetY, width, height, tileName, OGRDS):
    sx = levelMosaicInfo.scaleX * 2
    sy = levelMosaicInfo.scaleY * 2
...
def buildPyramid(g, minfo, createdTileIndexDS, tileWidth, tileHeight, overlap, totalProcs):
...
def buildPyramidLevel(g, levelMosaicInfo, levelOutputTileInfo, level):
    ...
    """
    try:
        procYRange = list(split(range(1, levelOutputTileInfo.countTilesY + 1), g.totalProcs))
        thisProcYRangeStart = procYRange[g.thisProc-1][0]
        thisProcYRangeEnd = procYRange[g.thisProc-1][-1]
    except IndexError:
        return

    if np.array(procYRange[g.thisProc - 1]).size:
        ...
        print("process %d/%d: Rows %d-%d of level %s complete." % (
        g.thisProc, g.totalProcs, thisProcYRangeStart, thisProcYRangeEnd, str(level)))

        ##     check tile generation of all processes is complete before continuing on
        xIndex == levelOutputTileInfo.countTilesX
        for procIndex in range(1, g.totalProcs + 1):
            if procIndex == g.thisProc:
                continue
            if np.array(procYRange[procIndex - 1]).size:
                endy = procYRange[procIndex - 1][-1]
                last_tile_in_proc_yrange = getTileName(g, levelMosaicInfo, levelOutputTileInfo, xIndex, endy, level)
                while not os.path.isfile(last_tile_in_proc_yrange):
                    time.sleep(30)
                    print("level %s: waiting for last tile in process %d/%d to complete..." % (
                    str(level), procIndex, g.totalProcs))
        ...
        for yIndex in yRange:
            ...
            for xIndex in xRange:
                ...
                if xIndex == levelOutputTileInfo.countTilesX:
                    ...
                    width = levelOutputTileInfo.tileWidth
                tilename = getTileName(g, levelMosaicInfo, levelOutputTileInfo, xIndex, yIndex, level)
                createPyramidTile2(g, levelMosaicInfo, offsetX, offsetY, width, height, tilename, OGRDS)
...
def Usage(isError):
    ...
    print("        [-nproc <number_of_processes>] [-proc <process_id>]", file=f)
...
def main(args=None, g=None):
    ...
    while i < len(argv):
        ...
        if arg == "--help":
            ...
            i += 1
            g.totalProcs = int(argv[i])
        elif arg == "-proc":
            i += 1
            g.thisProc = int(argv[i])    
        elif arg[:1] == "-":
    ...
    ti = tile_info(minfo.xsize, minfo.ysize, g.TileWidth, g.TileHeight, g.Overlap,g.totalProcs)
    ...
    if g.Levels > 0:
        buildPyramid(g, minfo, dsCreatedTileIndex, g.TileWidth, g.TileHeight, g.Overlap, g.totalProcs)
